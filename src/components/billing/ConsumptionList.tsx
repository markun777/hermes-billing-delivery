...
export function ConsumptionList({ packageName, expiry, models, className }: ConsumptionListProps) {
  return (
    <section
      className={cn(
        'flex h-[320px] w-full flex-col items-start gap-2 rounded-[4px] border border-[rgba(188,205,232,0.56)] bg-white px-3 py-5',
        className
      )}
    ...
    >
      ...
      {models.map(model => (
        <div className="flex h-10 w-full items-center gap-6 rounded-[4px] px-4 py-2" key={model.id}>
          <div className="flex w-[220px] shrink-0 items-center gap-2">
            <img alt="" className="size-4" src="/billing-assets/model-provider.svg" />
